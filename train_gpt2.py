...
def get_lr(it):
    ...
    assert 0 <= decay_ration <= 1, f"decay_ratio: {decay_ration}"
...
for step in range(max_steps):
    ...
    if master_process:
        print(f'step {step:4d}, loss: {loss_accum.item()}, lr: {lr:.4e}, norm: {norm:.4f} , dt: {dt*1000:.2f}ms, tokens/sec: {tokens_per_sec:.2f}')
if ddp:
